  ...
  return <header className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-sm py-3' : 'bg-transparent py-5'}`}>
        ...
        <nav className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <div className="flex items-center space-x-6">
              {navLinks.map(link => <Link key={link.path} to={link.path} className={`text-sm font-medium transition-colors hover:text-gray-500 ${location.pathname === link.path ? 'text-black' : 'text-gray-600'}`}>
                  {link.name}
                </Link>)}
            </div>
      ...
      {isMenuOpen && <div className="md:hidden bg-white absolute top-full left-0 w-full shadow-lg py-4 px-6">
          <div className="flex flex-col space-y-4">
            {navLinks.map(link => <Link key={link.path} to={link.path} onClick={closeMenu} className={`text-sm font-medium py-2 transition-colors hover:text-gray-500 ${location.pathname === link.path ? 'text-black' : 'text-gray-600'}`}>
                {link.name}
              </Link>)}
